...
import { IconHome, IconCurriculum, IconMonitor, IconStudents, IconSettings, IconLogout, IconChevronsLeft, IconRobot, IconActivity, IconCalendar, IconHuman, IconFileText, IconMail, IconTruck, IconMessageSquare, IconShield, IconGavel, IconLink, IconPhone, IconAnnouncement } from './icons';
...
import AdminLetterTrack from './admin/AdminLetterTrack';
import AdminFedExTracking from './admin/AdminFedExTracking';
import AdminConversations from './admin/AdminConversations';
import AdminDirectory from './admin/AdminDirectory';
import AdminCallLogs from './admin/AdminCallLogs';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ setView, chatLog, studentPhotos, liveSessions, enterInstructorMode, enterAdminMode, courses, setCourses, students, instructors, scheduledClasses, onAddStudent, onUpdateStudent, onToggleInactiveStatus, onRescheduleStudent, onViewStudent, onRefundPayment, onUploadCertificate, currentUser, users, onAddUser, onUpdateUser, onRemoveUser, onAddScheduledClass, onUpdateScheduledClass, onDeleteScheduledClass, onAddInstructor, onUpdateInstructor, onDeleteInstructor, onMarkAdminNotificationRead, onMarkAllAdminNotificationsRead, onSendChatMessage, onRemoveStudent, onManualExpedite, coupons, onAddCoupon, onDeleteCoupon, websiteContent, onUpdateWebsiteContent }) => {
    ...
    const getAdminViewTitle = (view: AdminView): string => {
        switch (view) {
            ...
            case AdminView.LetterTrack: return "Letter Track Pro";
            case AdminView.FedExTracking: return "FedEx Real-Time Tracking";
            case AdminView.Conversations: return "Unified Inbox (Email & SMS)";
            case AdminView.Directory: return "Legal & Probation Directory";
            case AdminView.CallLogs: return "Telzio Call Dashboard";
            case AdminView.Advertisements: return "Marketing Studio";
    ...
    const renderAdminView = () => {
        switch (adminView) {
            ...
            case AdminView.Curriculum:
                return <AdminCurriculum courses={courses} setCourses={setCourses} scheduledClasses={scheduledClasses} onAddScheduledClass={onAddScheduledClass} onUpdateScheduledClass={onUpdateScheduledClass} onDeleteScheduledClass={onDeleteScheduledClass} instructors={instructors} />;
            ...
            case AdminView.Students:
                return <AdminStudents onViewStudent={handleViewStudentProfile} courses={courses} students={students} onAddStudent={onAddStudent} onToggleInactiveStatus={onToggleInactiveStatus} onUpdateStudent={onUpdateStudent} scheduledClasses={scheduledClasses} />;
            ...
            case AdminView.LetterTrack:
                return <AdminLetterTrack />;
            case AdminView.FedExTracking:
                return <AdminFedExTracking />;
            case AdminView.Conversations:
                return <AdminConversations />;
            case AdminView.Directory:
                return <AdminDirectory students={students} onViewStudent={handleViewStudentProfile} />;
            case AdminView.CallLogs:
                return <AdminCallLogs students={students} />;
            case AdminView.Advertisements:
                return <AdminAdvertisements courses={courses} websiteContent={websiteContent} />;
        ...
        <div className="bg-gray-100 min-h-screen">
            <aside className={`fixed top-0 left-0 h-full z-10 transition-all duration-300 bg-white border-r border-gray-200 p-4 flex flex-col ${isSidebarMinimized ? 'w-20' : 'w-64'}`}>
                ...
                <nav className="flex-1 space-y-1 overflow-y-auto pr-1 custom-scrollbar">
                    ...
                    <NavLink icon={<IconAnnouncement />} label="Marketing Studio" view={AdminView.Advertisements} activeView={adminView} onClick={setAdminView} />
                    ...
                    <NavLink icon={<IconPhone />} label="Call Logs" view={AdminView.CallLogs} activeView={adminView} onClick={setAdminView} />
                    <NavLink icon={<IconShield />} label="Directory" view={AdminView.Directory} activeView={adminView} onClick={setAdminView} />
                    <NavLink icon={<IconHuman />} label="Instructors" view={AdminView.Instructors} activeView={adminView} onClick={setAdminView} />
                    <NavLink icon={<IconMessageSquare />} label="Inbox" view={AdminView.Conversations} activeView={adminView} onClick={setAdminView} />
                    <NavLink icon={<IconMail />} label="Letter Tracking" view={AdminView.LetterTrack} activeView={adminView} onClick={setAdminView} />
                    {currentUser.role === UserRole.Admin && (
